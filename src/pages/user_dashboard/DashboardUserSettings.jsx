...
import { useState } from 'react'
 ...
 const SwitchButton = ({toggleNotification, label, toggleValue}) => {
  return <div htmlFor="switch" className='flex flex-row mb-12'>
    <button className='w-14 h-6 py-1 bg-white rounded-xl mr-4 flex items-center' onClick={toggleNotification}>
      <div className={`w-3 h-3 rounded-full transition duration-500 bg-${toggleValue ? 'primary40 translate-x-11' : "gray-300"} `} >
      </div>
    </button>
    <label htmlFor="check" className='text-white text-sm'>{label}</label>
  </div>
}
...
const NewsAndUpdateSettings = () => {
  ...
  return <div className='bg-[#005232] p-2'>
                ...
                <div className='px-4 mt-10 w-full mr-40'>
                  ...
                  <ul className='w-full'>
                  {
                      SettingsData.map((item, index) => 
                        <Link to={item.user_a_link} key={index} onClick={() => setComponentToDisplay(index+1)} className=' flex flex-row bg-[rgba(0, 109, 68, 0.01)] border-2 border-[#ECECEC]  p-3 mb-4 items-center rounded-lg'>
